...
import time
import csv
class ELU_Plus_2d_Hybrid_3D_2D(layers.Module): 
    # Input layers.Module 
    #
    #
    #
    ...
    def __init__(self):
        ...
        super(ELU_Plus_2d_Hybrid_3D_2D, self).__init__() # 
        ...
        self.cn4_2d_1 = layers.Conv2d(in_channels = 32, out_channels = 64, kernel_size = 3, padding = 1)
        self.cn4_2d_2 = layers.Conv2d(in_channels = 64, out_channels = 128, kernel_size = 3, padding = 1)
        ...
        self.fc5 = layers.Linear(128 * 64 * 64 ,256)
    ...
    def forward(self,x):
        ...
        output = self.cn4_2d_1(output)
        
        output = F.elu(output)
        output = self.cn4_2d_2(output)
...
def train(model,device,the_dataloader, optim,epochs):
    ...
    for epoch in range(epochs):
        ...
        Regular_Plus_2d_3D_2D_Hybrid_Time = open(('Regular_Plus_2d_3D_2D_Hybrid_Training_Time.txt'), "w" ) 
        Regular_Plus_2d_3D_2D_Hybrid_Time.write("Training time:" + str(execution_time) + "s" + "\n") 
        Regular_Plus_2d_3D_2D_Hybrid_Time.close() 

        Regular_Plus_2d_3D_2D_Hybrid_Testing_Loss = open(("Regular_Plus_2d_3D_2D_Hybrid_Training_Loss.csv"), "w" ,newline='') # opens the csv
        csvWritingFileObject = csv.writer(Regular_Plus_2d_3D_2D_Hybrid_Testing_Loss) # creates an instance of teh class that will write to teh csv
        ...
        Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values = open(('Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values_Training.txt'), "w" ) 
        Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values.write("Predicted values: " + str(predicted_y) + "\n" + "Actual values:" + str(actual_y)) 
        Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values.close() 
...
def test(model,device,the_dataloader):
    ...
    end_time = time.time()
    execution_time = end_time - start_time
    Regular_Plus_2d_3D_2D_Hybrid_Time = open(('Regular_Plus_2d_3D_2D_Hybrid_Time.txt'), "w" ) 
    Regular_Plus_2d_3D_2D_Hybrid_Time.write("Testing time:" + str(execution_time) + "s" + "\n") 
    Regular_Plus_2d_3D_2D_Hybrid_Time.close() 

    Regular_Plus_2d_3D_2D_Hybrid_Testing_Loss = open(("Regular_Plus_2d_3D_2D_Hybrid_Testing_Loss.csv"), "w" ,newline='') # opens the csv
    csvWritingFileObject = csv.writer(Regular_Plus_2d_3D_2D_Hybrid_Testing_Loss) # creates an instance of teh class that will write to teh csv
    rowOfData = [batch_list,loss_list] # image details to be added to teh csv
    csvWritingFileObject.writerow(rowOfData) 
    
    # predicted and actual values for confusion matrix
    Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values = open(('Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values_Testing.txt'), "w" ) 
    Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values.write("Predicted values: " + str(predicted_y) + "\n" + "Actual values:" + str(actual_y)) 
    Regular_Plus_2d_3D_2D_Hybrid_Conf_Met_Values.close() 
...
Regular_Plus_2dModel = ELU_Plus_2d_Hybrid_3D_2D()




optim = optimizers.SGD(Regular_Plus_2dModel.parameters(),lr=0.0005)# transfer learning paper > used 0.0005 for transfer learning 
...
pretrained3D_2D_Hybrid_Regular_Plus_2d = test(pretrained3D_2D_Hybrid_Regular_Plus_2d, 'cpu',load_testing_data)
